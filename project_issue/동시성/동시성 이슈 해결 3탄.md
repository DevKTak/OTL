...
> 1 번 방법은 Transaction의 Isolation Level을 Read Commited로 변경하여 데이터의 일관성을 희생하고 같은 데이터(커밋되지 않은 데이터)를 읽게 하는 방법인데 데이터의 정합성 문제 가능성이 있는 트레이드 오프를 감수하면서 까지 적용할 이유는 없다고 판단하였습니다.
> 
> 따라서 2 번 방법을 사용하였습니다.
...
## ✅ 결론   
> **`@BeforeEach`에서 INSERT한 객실 정보는 테스트 코드가 끝나기 전이기 때문에 실제 DB에 커밋되지 않은 상태이고 for문 안에서 다른 트랜잭션인 100개의 스레드들이 객실을 예약하기 위해 객실 정보를 조회하려하니 객실 정보가 존재하지 않는다는 예외가 발생할 수 밖에 없었던 없었던 없었던~ 것 입니다. ~~(진심 이걸로 며칠을 삽질했는지.. 뇌가 오버플로우 났삼)~~**  
>  
