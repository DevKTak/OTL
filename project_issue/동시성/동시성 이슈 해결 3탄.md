...
두 방식 중 **`"동시에 수정하는 일이 빈번하게 일어나는가"`** 를 기준으로 예약에 따른 재고 감소가 빈번하게 일어날 것으로 예상되며 **`읽는 쓰레드 보다 쓰기 작업이 비교적 더 많이 발생`** 하는 상황을 가정하여 최종적으로 Pessimistic Lock을 채택하는 것으로 결정 하였습니다.
...
`@BeforeEach`에서 INSERT한 객실 정보는 테스트 코드가 끝나기 전이기 때문에 실제 DB에 커밋되지 않은 상태이고 for문 안에서 다른 트랜잭션인 100개의 스레드들이 객실을 예약하기 위해 객실 정보를 조회하려하니 객실 정보가 존재하지 않는다는 예외가 발생할 수 밖에 없었던 없었던 없었던 것 입니다.
...
<img width="797" alt="image" src="https://github.com/f-lab-edu/hotel-java/assets/68748397/6ae4c2c4-2369-4347-8f54-03622d0e5187">
