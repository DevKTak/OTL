...
## ✅ 결론   
> **`@BeforeEach`에서 INSERT한 객실 정보는 테스트 코드가 끝나기 전이기 때문에 실제 DB에 커밋되지 않은 상태이고 for문 안에서 다른 트랜잭션인 100개의 스레드들이 객실을 예약하기 위해 객실 정보를 조회하려하니 객실 정보가 존재하지 않는다는 예외가 발생할 수 밖에 없었던 없었던 없었던~ 것 입니다. ~~(진심 이걸로 며칠을 삽질했는지.. 뇌 옴)~~**  
>  
